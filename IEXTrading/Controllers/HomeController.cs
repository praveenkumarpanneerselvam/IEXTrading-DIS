    ...
    public class HomeController : Controller
    {
        ...
        ****/
        public IActionResult Refresh(string tableToDel)
        {
            ...
            tableCount.Add("Strategy Suggestions", dbContext.Output.Count());
        ...
        ****/
        ...
        public IActionResult SaveSuggestions()
        {
            List<StrategyResponse> responses = JsonConvert.DeserializeObject<List<StrategyResponse>>(TempData["Quotes"].ToString());
            foreach (StrategyResponse response in responses)
            {
                //Database will give PK constraint violation error when trying to insert record with existing PK.
                //So add company only if it doesnt exist, check existence using symbol (PK)
                if (dbContext.Output.Where(c => c.symbol.Equals(response.symbol)).Count() == 0)
                {
                    dbContext.Output.Add(response);
                }
            }
            dbContext.SaveChanges();
            ViewBag.dbSuccessComp = 1;
            return View("GetStrategyResults", responses);
        }
        ...
        ****/
        public void ClearTables(string tableToDel)
        {
            if ("all".Equals(tableToDel))
            {
                ...
                dbContext.Output.RemoveRange(dbContext.Output);
            }
            ...
            else if ("Strategy Suggestions".Equals(tableToDel))
            {
                dbContext.Output.RemoveRange(dbContext.Output);
            }
            dbContext.SaveChanges();
